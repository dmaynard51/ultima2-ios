...
**1. Get the app.** Download **[Ultima II.ipa](https://github.com/dmaynard51/ultima2-ios/releases/latest)** from Releases.
...
Ultima II DOS files — the folder that contains **`ULTIMAII.EXE`** (e.g. from your GOG copy). Copy them
onto the phone by either:
- **Files app → On My iPhone → Ultima II** → paste your game files in, **or**
- **iTunes / Finder → phone → File Sharing → Ultima II** → drag the files into its Documents box.

> ⚠️ **Put the files LOOSE at the root — not inside a folder.** That Ultima II folder *is* the
> game's `C:\` drive, and the app auto-runs `ULTIMAII.EXE` from `C:\`. If you drag a *folder* in
...
> → **Move** → up one level into the **Ultima II** folder.

**4. Open Ultima II** — it boots straight into the game (landscape, D-pad + command keyboard, sound).
...
There's no custom Android app, but Ultima II runs well on Android through an existing DOSBox
...
3. Put your Ultima II game folder on the phone — the one with **`ULTIMAII.EXE`** — or zip that folder.
4. **Load Content** → pick the folder (or the `.zip`) → run it with **DOSBox-Pure**. It
   mounts your files as `C:` and shows a start menu — choose **`ULTIMAII.EXE`**.
...
2. New game → set the **`C:`** mount to your Ultima II folder and the run command to **`ULTIMAII.EXE`**.
