...
#
# The *complete* Ultima II (the real DOS game in DOSBox), built on litchie/dospad
# (the open-source iOS DOSBox, GPLv2) — cloned + patched at build time, not
# re-hosted here — and your own Ultima II data (never committed).
#
# Prereqs:
#   - Xcode (signed in with the Apple ID that owns your team).
#   - iPhone/iPad connected, unlocked, "Trust" accepted, Developer Mode on.
#   - Your Ultima II game folder (the one with ULTIMAII.EXE). On a Mac GOG install
#     that's usually /Applications/Ultima II™.app/Contents/Resources/game
#
# Usage: dosbox/build-ios-dosbox.sh <AppleTeamID> [/path/to/ultimaII/gamedata]
# Find your Team ID: security find-identity -v -p codesigning (the code in parens).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TEAM="${1:?Usage: build-ios-dosbox.sh <AppleTeamID> [ultimaII-data-dir]}"
SRC="${2:-/Applications/Ultima II™.app/Contents/Resources/game}"
BUNDLE_ID="${U2DOS_BUNDLE_ID:-info.u2redux.u2dos}"
RUN_EXE="${U2DOS_RUN_EXE:-ULTIMAII.EXE}"     # the DOS program dospad auto-runs
APP_NAME="Ultima II"
ICON="ultima2-icon.png"
