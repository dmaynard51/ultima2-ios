#!/bin/bash
# Build a one-tap "Ultima II in DOSBox" app for your iPhone/iPad and install it,
# with YOUR copy of the game.
#
# This is the *complete* Ultima II (the real DOS game running in DOSBox), unlike
...
# your own Ultima II data (never committed).
#
# Prereqs:
...
#   - Your Ultima II game folder (the one with ULTIMAII.EXE). On a Mac GOG
#     install that's usually /Applications/Ultima II™.app/Contents/Resources/game
#
...
U2_SRC="${2:-/Applications/Ultima II™.app/Contents/Resources/game}"
BUNDLE_ID="${U2DOS_BUNDLE_ID:-info.u2redux.u2dos}"
WORK="${HOME}/Library/Caches/u2-dosbox"
DOSPAD="$WORK/dospad"
...
if [ ! -f "$U2_SRC/ULTIMAII.EXE" ] && [ ! -f "$U2_SRC/ultimaii.exe" ]; then
  echo "ERROR: no Ultima II data at:" >&2
  echo "  $U2_SRC" >&2
  echo "Pass the folder with ULTIMAII.EXE as the 2nd argument." >&2
  exit 1
fi
...
#     Set U2DOS_KEEP_THUMBNAIL=1 to keep it (e.g. you have a paid account and want the
#     Files thumbnails). This only edits references, and aborts untouched if anything
#     unexpected is found, so it's safe.
if [ -z "${U2DOS_KEEP_THUMBNAIL:-}" ]; then
...
# 3. Patch: auto-run ULTIMAII.EXE from the C-drive root on launch (one-tap boot).
EMU="$DOSPAD/dospad/Main/DOSPadEmulator.m"
if ! grep -q "Ultima II one-tap" "$EMU"; then
  echo "Patching dospad to auto-run Ultima II ..."
  python3 - "$EMU" <<'PY'
import sys
p = sys.argv[1]
s = open(p).read()
...
inject = marker + '''
    ...
    // Ultima II one-tap boot: if ULTIMAII.EXE is present at the C drive root, run it
    // directly (dedicated-app behaviour, independent of package-type detection).
    {
        DPDrive *cDrive = [self.package findDrive:'C'];
        if (cDrive) {
            NSString *u2exe = [cDrive.sourceUrl.path stringByAppendingPathComponent:@"ULTIMAII.EXE"];
            if ([[NSFileManager defaultManager] fileExistsAtPath:u2exe]) {
                [self.commandList addObject:@"C:"];
                [self.commandList addObject:@"ULTIMAII.EXE"];
            }
        }
    }'''
...
# 3b. Ultima II branding: swap the app icon (gold ankh) and rename to "Ultima II".
MASTER="$SCRIPT_DIR/ultima2-icon.png"
ICON_SET="$DOSPAD/Resources/Assets.xcassets/AppIcon.appiconset"
if [ -f "$MASTER" ] && [ -d "$ICON_SET" ]; then
  echo "Applying the Ultima II app icon ..."
...
plutil -replace CFBundleDisplayName -string "Ultima II" "$DOSPAD/Resources/iDOS-Info.plist" 2>/dev/null || true
...
# 3f. Lock the app to landscape. Ultima II is a 4:3 landscape game, and only the landscape
...
\$99 Developer Program. Do this once (takes 2 minutes):
  ...
  5. Then re-run this script to copy your Ultima II data onto the device
...
cp "$U2_SRC"/* "$STAGE/" 2>/dev/null || find "$U2_SRC" -maxdepth 1 -type f -exec cp {} "$STAGE/" \;
rm -f "$STAGE/manual.pdf" "$STAGE"/dosbox*.conf 2>/dev/null || true
printf '{\n  "name": "Ultima II",\n  "autorun": "ULTIMAII.EXE"\n}\n' > "$STAGE/idos.json"
echo "Copying your Ultima II data onto the device ..."
...
# 8. Launch — boots straight into Ultima II.
...
echo "  VPN & Device Management, then reopen. It boots straight into Ultima II."
